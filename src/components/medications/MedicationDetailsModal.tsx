      ...
      <div className="modal-backdrop" onClick={onClose}>
        <div className="modal-dialog" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '640px' }}>
          ...
          <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', maxHeight: '70vh', overflowY: 'auto' }}>
            ...
            {/* Large Photo Preview if attached - Clickable to Enlarge */}
            {medication.imageUrl && (
              <div
                style={{
                  textAlign: 'center',
                ...
                onClick={() => setIsPhotoLightboxOpen(true)}
                title="Tap to enlarge full screen"
              >
                <img
                  src={medication.imageUrl}
                  alt={medication.name}
                ...
                />
                <div className="text-xs text-primary font-semibold" style={{ marginTop: '0.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.35rem' }}>
                  <span>🔍 Tap photo to view full-screen / zoom label</span>
                </div>
              </div>
            )}
